...
while IFS= read -r raw || [[ -n "$raw" ]]; do
  ...
  if [[ $in_block -eq 0 ]] && echo "$line" | grep -qE '^=== delete:'; then deletes_todo+=( "$(norm_path "$(trim "$(echo "$line" | sed -E 's/^=== delete:[[:space:]]*(.+)[[:space:]]*===$/\1/')")")" ); continue; fi
  if [[ $in_block -eq 0 ]] && echo "$line" | grep -qE '^=== mv:'; then from="$(trim "$(echo "$line" | sed -E 's/^=== mv:[[:space:]]*(.+)[[:space:]]*=>[[:space:]]*(.+)[[:space:]]*===$/\1/')")"; to="$(trim   "$(echo "$line" | sed -E 's/^=== mv:[[:space:]]*(.+)[[:space:]]*=>[[:space:]]*(.+)[[:space:]]*===$/\2/')")"; moves_from+=( "$(norm_path "$from")" ); moves_to+=( "$(norm_path "$to")" ); continue; fi
  if [[ $in_block -eq 0 ]] && echo "$line" | grep -qE '^=== block:'; then cur_path="$(norm_path "$(trim "$(echo "$line" | sed -E 's/^=== block:[[:space:]]*([^#[:space:]]+)\#.*$/\1/')")")"; cur_anchor="$(trim "$(echo "$line" | sed -E 's/^=== block:[[:space:]]*[^#]+#([A-Za-z0-9_-]+).*$/\1/')")"; cur_mode="$(trim "$(echo "$line" | sed -E 's/^.*mode=(replace|append|prepend|append_once).*$/\1/')" )"; [[ -z "$cur_mode" ]] && cur_mode="replace"; cur_index="$(echo "$line" | sed -nE 's/^=== block:[[:space:]]*[^#]+#[^[:space:]@]+@index=([0-9]+).*$/\1/p')"; [[ -z "$cur_index" ]] && cur_index="1"; cur_tmp="$(mktemp)"; in_block=2; continue; fi
  if [[ $in_block -eq 0 ]] && echo "$line" | grep -qE '^=== diff:'; then cur_diff_opts="$(trim "$(echo "$line" | sed -E 's/^=== diff:[[:space:]]*(.+)[[:space:]]*===$/\1/')" )"; cur_tmp="$(mktemp)"; in_block=3; continue; fi
  if [[ $in_block -eq 1 && "$line" == "=== end ===" ]]; then files_todo+=( "$cur_path" ); files_tmp+=( "$cur_tmp" ); in_block=0; cur_path=""; cur_tmp=""; continue; fi
  if [[ $in_block -eq 2 && "$line" == "=== end ===" ]]; then blocks_path+=( "$cur_path" ); blocks_anchor+=( "$cur_anchor" ); blocks_mode+=( "$cur_mode" ); blocks_tmp+=( "$cur_tmp" ); blocks_index+=( "$cur_index" ); in_block=0; cur_path=""; cur_tmp=""; cur_anchor=""; cur_mode=""; cur_index=""; continue; fi
  if [[ $in_block -eq 3 && "$line" == "=== end ===" ]]; then diffs_tmp+=( "$cur_tmp" ); diffs_opts+=( "$cur_diff_opts" ); in_block=0; cur_tmp=""; cur_diff_opts=""; continue; fi
...
PY
      then out="$(mktemp)"; sed -n "1,${start}p" "$file" >"$out"; cat "$body_file" >>"$out"; sed -n "${end},\$p" "$file" >>"$out"; mv -f "$out" "$file"; rm -f "$head_file" "$body_file" "$tail_file" "$norm_content"; log "ℹ️ append_once：内容已存在，跳过（$rel #$anchor @index=$index）"; return 0; fi
    fi
    sed -n "1,${start}p" "$file" >"$out"; printf '%s\n' "$begin_line" >>"$out"
    case "$mode" in replace) cat "$norm_content" >>"$out" ;; append) cat "$body_file" >>"$out"; cat "$norm_content" >>"$out" ;; append_once) cat "$body_file" >>"$out"; cat "$norm_content" >>"$out" ;; prepend) cat "$norm_content" >>"$out"; cat "$body_file" >>"$out" ;; *) cat "$norm_content" >>"$out" ;; esac
    printf '%s\n' "$end_line" >>"$out"; sed -n "${end},\$p" "$file" >>"$out"
    mv -f "$out" "$file"; rm -f "$head_file" "$body_file" "$tail_file" "$norm_content"; log "🧩 命中锚区：$rel #$anchor (mode=$mode)"; return 0
  }
  if locate_and_apply "$file" "$anchor" "$mode" "$content_file" "$index"; then return 0; fi
  local booted=0; { printf '\n<!-- XGIT:BEGIN %s -->\n' "$anchor"; printf '<!-- XGIT:END %s -->\n' "$anchor"; } >>"$file" && booted=1; log "ℹ️ 自动引导空锚点：$rel #$anchor (@index=$index)"
...
PY
  ...
  if [[ "${total:-0}" -ge 1 ]]; then if locate_and_apply "$file" "$anchor" "$mode" "$content_file" "$total"; then return 0; fi; fi
