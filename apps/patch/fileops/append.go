...
func FileAppend(repo, rel string, data []byte, logger DualLogger) error {
	...
	if err := os.MkdirAll(filepath.Dir(abs), 0o755); err != nil {
		if logger != nil {
			logger.Log("❌ file.append mkdir 失败：%s (%v)", rel, err)
		}
		return err
	}
	...
	if err != nil {
		if logger != nil {
			logger.Log("❌ file.append 打开失败：%s (%v)", rel, err)
		}
		return err
	}
	defer f.Close()
	...
	if _, err := f.Write(b); err != nil {
		if logger != nil {
			logger.Log("❌ file.append 写入失败：%s (%v)", rel, err)
		}
		return err
	}
	if logger != nil {
		logger.Log("✅ file.append 完成：%s", rel)
	}
